...
func Range(start time.Time, end time.Time, step time.Duration) iterator.Iterator[time.Time] {
	...
	if asc {
		return iterator.Pipe(Ascending(start, step), iterator.LimitFunc(func(_ uint, item time.Time) (bool, error) {
			return !end.Before(item), nil
		}))
	} else {
		return iterator.Pipe(Descending(start, step), iterator.LimitFunc(func(_ uint, item time.Time) (bool, error) {
